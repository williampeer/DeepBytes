...
def training_and_recall_hpc_helper(hpc, training_set_size, train_set_num, original_training_patterns):
    ...
    hpc_learn_patterns_wrapper(hpc, training_patterns=training_set, max_training_iterations=5)  # when training is fixed,
    ...
    # Show recalled patterns:
    print "Displaying chaotically recalled patterns..."
    for pat in hippocampal_chaotic_recall_patterns:
        Tools.show_image_from(pat)

    return [hippocampal_chaotic_recall_patterns, random_ins]


def pseudopattern_I_helper_new_random_in_convergence_patterns(hpc, desired_set_size):
    ...
    for i in range(desired_set_size):
        ...
        _, _, hpc_stable_output = hpc.recall_until_stability_criteria(False, 300)
        current_pseudopatterns_I.append([random_hpc_input, hpc_stable_output])

    return current_pseudopatterns_I
...
def experiment_4_x_2(hpc, ann, training_set_size, original_training_patterns):
    ...
    for train_set_num in range(5):  # always five training sets
        ...
        for recalled_pattern_index in range(len(current_set_hipp_chaotic_recall)):
            chaotically_recalled_patterns.append(current_set_hipp_chaotic_recall[recalled_pattern_index])
            current_pseudopatterns_II.append([current_set_hipp_chaotic_recall[recalled_pattern_index],
                                              current_set_random_ins[recalled_pattern_index]])
        ...
        if len(current_set_random_ins) > 0:
            ...
            # adding up to the desired number of pseudopatterns of type I to the set from the chaotically recalled ones
            for pattern_ctr in range(pseudopattern_set_size/2):
            ...
            # current_pseudopatterns_I = pseudopattern_I_helper_new_random_in_convergence_patterns(
            #         desired_set_size=pseudopattern_set_size/2)
